...
class CtrPeriod {
    ...
    static async add(global, args) {
        ...
        let exist = await Helper.validateIfExist({
            ...
            collectionName,
            params: {
                dateFrom: new Date(args.input.dateFrom),
                dateUp: new Date(args.input.dateUp)
            }
        });
        if (exist) {
            return {
                status: false,
                message: `The period alredy exist!`,
                _id: null
            };
        }
        try {
